...
<<<<<<< HEAD:azure-storage-cpp-lite/src/blob/blob_client_wrapper.cpp
...
>>>>>>> 8c4db0fdc03a29fc9c5e18711fa05fa0d4cdb04d:cpplite/src/blob/blob_client_wrapper.cpp
        {
            char *buffer = new char[s_block_size];
            return buffer;
        }
...
>>>>>>> 8c4db0fdc03a29fc9c5e18711fa05fa0d4cdb04d:cpplite/src/blob/blob_client_wrapper.cpp
    ...
    try
    {
        ...
        std::shared_ptr<storage_account> account = std::make_shared<storage_account>(accountName, cred, use_https, blob_endpoint);
        std::shared_ptr<storage_client_key> client_key = std::make_shared<storage_client_key>(encryption_key, encryption_key_sha256, encryption_algorithm);
        std::shared_ptr<blob_client> blobClient = std::make_shared<microsoft_azure::storage::blob_client>(account, concurrency_limit, client_key);
        errno = 0;
        return blob_client_wrapper(blobClient);
    }
    catch (const std::exception &ex)
